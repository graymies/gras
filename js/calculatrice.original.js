...
CALCULATRICE = function(options)
{
  var fnc = prototype = {
	...
	initialize: function(options) {
		...
		if(this.idPartenaire == null){this.idPartenaire = 'generique';}
	...
	getIdPartenaire : function(pPartenaire){
		var idPartenaire;
		if(!isNaN(pPartenaire)){ //à MAJ avec Borg/part 3cb
			switch(pPartenaire){
				case "08000071690": idPartenaire = 'priceminister';//priceminister
							   		break;
			   	case "08000010430": idPartenaire = 'allopneus';//allopneus
									break;
			   	default:			idPartenaire = 'generique';
			}
		}else{
	...
	parseConfigPartenaire : function(data){
		...
		if(bannierePartenaire.attr('source').length != 0){
			if(bannierePartenaire.attr('cible').length != 0){banniereContent = '<a href="'+bannierePartenaire.attr('cible')+'" title="">';}
			banniereContent +='<img src="images/bannieres/'+bannierePartenaire.attr('source')+'" alt="" />';
			if(bannierePartenaire.attr('cible').length != 0){banniereContent += '</a>';}
		...
		if(xmlDataPartenaire.find('fonctionnement etape').length == 0){$('#beneficier').remove();}
		if(xmlDataPartenaire.find('avantages avantage').length == 0){$('#avantages').remove();}
	...
	verifMontant : function(){
		...
		if((obj.montant >= montantMin) && (obj.montant <= montantMax)){return true; }else{return false};
	},
	...
	messageErreur : function(){
		...
		var message = "";
		switch(obj.option){
			case "5": message = 'Pour le paiement en petites mensualités, le montant doit être compris entre '+montantMin+'€ et '+montantMax+'€';
					break;
			case "0": message = 'Pour le paiement "Comptant", le montant doit être compris entre '+montantMin+'€ et '+montantMax+'€';
					break;
			default : message = 'Pour le paiement en "'+libelleOption+'", le montant doit être compris entre '+montantMin+'€ et '+montantMax+'€';
						break;
		}
	...
	moteurCalcul : function(){
		...
		if(obj.option == 3){ // Si petite mensualité
			var taux = xmlDataGeneral.find('tranche').attr('taux');
			obj.taeg = (obj.montant <= 1524) ? xmlDataGeneral.find('teg[id="taux1"]').attr('taux') : xmlDataGeneral.find('teg[id="taux2"]').attr('taux');
			var mensualite = ((taux / 100)*obj.montant).toFixed(2);
			if(mensualite < 15){mensualite = 15;}
			$('#optionContent p').html(mensualite+"€ par mois<br />TAEG révisable : "+obj.taeg+"%");
		}else if(obj.option == 0){ // Si paiement comptant
			$('#optionContent p').html("Je règle "+obj.montant+"€ en une seule fois");
		}else if( (obj.option == 1) || (obj.option == 2) ){ // 3xCB
			var ma_simu = '';			
			var mens_1 = 0;
			
			var frais = obj.truncateDecimals(obj.montant*coef, 2);
			var mens_n = obj.truncateDecimals(obj.montant/obj.nbEcheance, 2);
			mens_1 = Math.round( ((obj.montant-2*mens_n)+frais)*100)/100;
			
			montant_total_du = obj.truncateDecimals( (mens_1 + 2*mens_n), 2);
			
				console.log('************************************************************************************************************');
				console.log('****                                                                                                    ****');
				console.log("****    montant :: "+obj.montant+" // coef :: "+coef+" ==> frais :: "+frais );
				console.log("****    totalDu :: "+montant_total_du+" // 1ere mens :: "+mens_1+" // autresMens :: "+mens_n );
				console.log('****                                                                                                    ****');
				console.log('************************************************************************************************************');
			...
			$('#recap .frais-taux').html(coef*100);
...
$(document).ready(function() {
	calc = new CALCULATRICE({urlConfigGenerale : "xml/3cb/config-generale-3cb.xml", option : 1, montant : 300});
});